...
model_stats[nrow(model_stats) + 1, ] <- list(
  Model = "Linear Regression (Testing)",
...
model_stats[nrow(model_stats) + 1, ] <- list(
  ...
  Sensitivity = 0.99924,
  Specificity = 0.02099,
  AUC = 0.78, # Replace with actual AUC for XGBoost model #1
  Kaggle_Score = .739 # Replace with actual Kaggle Score for XGBoost model #1
)
...
model_stats[nrow(model_stats) + 1, ] <- list(
  Model = "XGBoost model #3 FINAL MODEL (Testing)",
  True_Negatives = 42345,
...
#skim on clean2
# clean2 %>% skim() %>% kable() #leaving out unless required for demo
...
# Updated obvious outliers. However to explore the problem more I am checking the skewness and NZV of the data
